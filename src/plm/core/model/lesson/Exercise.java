...
public abstract class Exercise extends Lecture {
	...
	public static enum WorldKind {

		INITIAL, CURRENT, ANSWER
	}

	public static enum StudentOrCorrection {
	...
	private Map<ProgrammingLanguage, List<SourceFile>> sourceFiles = new HashMap<>();

	public Map<String, Class<Object>> compiledClasses = new TreeMap<>(); /* list of entity classes defined in the lesson */
	...
	public void compileAll(LogWriter out, StudentOrCorrection whatToCompile) throws PLMCompilerException {
		...
		if (Game.getProgrammingLanguage().equals(Game.JAVA)) {
			...
			if (getSourceFiles().get(Game.JAVA) != null) {
				for (SourceFile sf : getSourceFiles().get(Game.JAVA)) {
			...
			try {
				...
				if (Game.getInstance().isDebugEnabled() && getSourceFiles().get(Game.JAVA) != null) {
					for (SourceFile sf : getSourceFiles().get(Game.JAVA)) {
			...
			List<SourceFile> sfs = getSourceFiles().get(Game.SCALA);
	...
	/**
	 * get the list of source files for a given language, or create it if not
	 * existent yet
	 */
	public List<SourceFile> getSourceFilesList(ProgrammingLanguage lang) {
		List<SourceFile> res = getSourceFiles().get(lang);
		if (res == null) {
			res = new ArrayList<>();
			getSourceFiles().put(lang, res);
		}
		return res;
	}
			...
			for (Entity old : current.getEntities()) {
				...
				if (lang.equals(Game.JAVA) || lang.equals(Game.SCALA)) {
					...
					if (whatToMutate == StudentOrCorrection.STUDENT) {
						...
						for (SourceFile sf : getSourceFiles().get(lang)) {
	 ...
	 */
	public Map<ProgrammingLanguage, List<SourceFile>> getSourceFiles() {
		return sourceFiles;
	}
}
